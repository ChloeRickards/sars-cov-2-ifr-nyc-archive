...
age_epidata <- read_csv("data/NYCHealth_cumul_data.csv") 
#age_epidata <- read_csv("data/NYCHealth_cumul_data_allDeaths.csv") 
...
# Adds age category for whole population, sums deaths/cases on those days
# age_epidata <- age_epidata %>%
#   rbind(age_epidata %>%
#           select(-age_class) %>%
#           group_by(date) %>%
...
age_popdata <- read_csv("data/NYC_pop_age_boro.csv")
...
prev_est_age <- read_csv("data/serosurvey_by_age_draws.csv") %>% 
  select(-X1) %>% 
  select(-age_midpoint) %>%
...
# adds "all" column with mean serosurvey and total pop
# prev_est_age <- prev_est_age %>%
#   rbind(data.frame(age_class = "all", 
#                    seropos = weighted.mean(prev_est_age$seropos, prev_est_age$pop), 
#                    pop = sum(prev_est_age$pop)))
...
age_trend_data <- read.csv("data/serop_age_spain.csv")
...
# scales serodata for the 55+ class, adds NYC
# I scale each draw by the scale from the linear model
# each new age class over 55 contains its own 5000 draws
...
age_popdata <- aggregate(cbind(NYC_Total=age_popdata$NYC_Total, Manhattan=age_popdata$Manhattan,
  ...
  # rbind(data.frame(age_class = "all", NYC_Total=sum(age_popdata$NYC_Total),
  #                  Manhattan=sum(age_popdata$Manhattan),Bronx=sum(age_popdata$Bronx),
  #                  Brooklyn=sum(age_popdata$Brooklyn),Queens=sum(age_popdata$Queens),
  #                  Staten_Island=sum(age_popdata$Staten_Island)))
...
#save(list = ls(), file = "data/data_setup_adj.rda")
...
save(list = ls(), file = "data/data_setup_adj.rda")
saveRDS(IFRs, file = paste0("results/age_stratified_IFRs_adj", suffix, ".rds"))
# saveRDS(IFRs, file = paste0("results/age_stratified_IFRs_adj_allDeaths", suffix, ".rds"))
