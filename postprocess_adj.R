...
load("data/data_setup_adj.rda")

# Load results
IFRs <- readRDS(paste0("results/age_stratified_IFRs_adj", suffix, ".rds"))
# IFRs <- readRDS(paste0("results/age_stratified_IFRs_adj_allDeaths", suffix, ".rds"))
...
knitr::kable(IFR_pvals,
             format = "latex",
             booktabs = T) %>% 
  write(file = paste0("results/IFRs_pvals_adj", suffix, ".tex"))
...
ggsave(p_data, filename = "results/fig/epidata_adj.png", width = 8, height = 7)
...
ggsave(p_ifrs, filename = "results/fig/ifr_posteriors_adj.png", width = 8, height = 4)
...
# saveRDS(IFR_stats_adj, file = "IFR_stats_adj_allDeaths.RDS")
...
# saveRDS(prev_est_age_adj, file = "prev_est_age_adj_allDeaths.RDS")
