...
# Load results
IFRs <- readRDS(paste0("age_stratified_IFRs_raw", suffix, ".rds"))
...
knitr::kable(IFR_pvals,
             format = "latex",
             booktabs = T) %>% 
  write(file = paste0("IFRs_pvals_raw", suffix, ".tex"))
...
ggsave(p_data, filename = "epidata_raw.png", width = 8, height = 7)
...
ggsave(p_ifrs, filename = "ifr_posteriors_raw.png", width = 8, height = 4)
