...
# Load results
IFRs <- readRDS(paste0("results/age_stratified_IFRs_raw", suffix, ".rds"))
# IFRs <- readRDS(paste0("results/age_stratified_IFRs_raw_allDeaths", suffix, ".rds"))
...
age_classes_raw <- c("[18, 45)", "[45, 65)", "[65, 75)", "[75, Inf)", "all")

IFRs <- IFRs %>% 
  mutate(age_class = factor(age_class, 
                            levels = age_classes_raw[c(1, 2, 3, 4, 5)]))
  
# Compute Bayesian p-values ----------------------------------------------------

ref_age <- "[18, 45)"    # reference age class -- this used to be [20, 50)
...
knitr::kable(IFR_pvals,
             format = "latex",
             booktabs = T) %>% 
  write(file = paste0("results/IFRs_pvals_raw", suffix, ".tex"))
...
stratified_data <- read_csv("data/NYCHealth_cumul_data.csv")
#stratified_data <- read_csv("data/NYCHealth_cumul_data_allDeaths.csv")
...
p_data <- ggplot(stratified_data, aes(x = date, y = Total, color = age_class)) +
  ...
  labs(x = "", y = "") +
  scale_color_discrete(labels= c("18-45", "45+"))

ggsave(p_data, filename = "results/fig/epidata_raw.png", width = 8, height = 7)
...
ggsave(p_ifrs, filename = "results/fig/ifr_posteriors_raw.png", width = 8, height = 4)
...
IFR_stats_raw <- IFR_stats

saveRDS(IFR_stats_raw, file = "IFR_stats_raw.RDS")
# saveRDS(IFR_stats_raw, file = "IFR_stats_raw_allDeaths.RDS")
...
prev_est_age_classes = c("18-45", "45-65", "65-75", "75+")
...
# to be passed to the rmd
prev_est_age_raw <- prev_est_age

saveRDS(prev_est_age_raw, file = "prev_est_age_raw.RDS") 
# saveRDS(prev_est_age_raw, file = "prev_est_age_raw_allDeaths.RDS") 
